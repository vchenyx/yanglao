...
var search_count_url = basePath + "order/getUndoneCount.do ";//查询数量
var search_list_url = basePath + "order/getDoneList.do";//查询数据集合
		...
		success:function(json){
			...
			for(var i in json){
				...
				html	+="<td>" + obj.orderNo + "</td>";
			 	...
			 	html	+="<td>" + obj.handleDept + "</td>";
			 	html	+="<td>" + obj.acceptPerson + "</td>";
			 	html	+="<td>" + (obj.state == 0 ? '初始录入' : obj.state == 0 ? '已派单' : '已接单') + "</td>";
				html	+="<td>" + (obj.serviceType == 0 ? '服务商' : '志愿者') + "</td>";
				html	+="<td>" + obj.serviceClassify + "-" + obj.serviceItem + "</td>";
				html	+="<td>" + longToDate(obj.serviceTime) + "</td>";
				html	+="<td>" + (obj.isUrgency == 0 ? '一般' : '紧急') + "</td>";
				html	+="<td>" + (obj.payType == 0 ? '现金' : '余额') + "</td>";
