...
var find_dept_byid= basePath +"pensionDept/findByDept.do";
var find_by_column= basePath +"pensionPeople/findBycolumn.do";
var deal_ids = "";
var dept_option="";
var dept_option2="";
init();
...
function searchDepartment(){
	$.ajax({
		...
		success:function(json){
			...
			for(var i in list){
				var dep = list[i];
				dept_option +="<option value='"+dep.id+';'+dep.name+"'>";
				for(var j=3;j<=dep.level;j++){
					dept_option += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
				}
				dept_option += dep.name+"</option>";
			}
		}
	});
}
...
function searchDepartmentInfo(){
	$.ajax({
		...
		success:function(json){
			var list = json.list;
			dept_option2 = "<option value=\"\">--请选择--</option><option value=\"\">--请选择--</option>";	
			for(var i in list){
				var dep = list[i];
				console.log(dep);
				dept_option2 +="<option value="+dep[0]+">"+dep[1]+"</option>";
				console.log();
			}
		}
	});
	...
	layui.use(['layer'], function() {
		...
		layer.open({
			...
			success: function(layero, index) {
				...
				body.find("#department2").html(dept_option2);
				body.find("#inp").click();
			},
				...
				$.ajax({
						...
						data:{
				        	...
				        	"idCard":body.find("#idCard").val(),
				          	"stationId":body.find("#department2").val(),
	...
	layui.use(['layer'], function() {
		...
		layer.open({
			...
			success: function(layero, index) {
				...
				var body = layer.getChildFrame('body', index);
				body.find("#department2").html(dept_option2);
				body.find("#peopleName").val(list[0].name);
				if(list[0].sex == 2){ 
				...
				for(var i = 0;i<option1.length;i++){
					if(list[0].peopleDeptId+";"+list[0].peopleDeptName == option1.eq(i).val()){
