...
$("#add").click(function(){
	layui.use(['layer'], function() {
		...
		layer.open({
			type: 2,
			title: '添加服务',
			content: basePath + 'static/temp/addServiceProvide.html',
			btn: ['添加', '取消'],
			area: ['550px', '450px'],
			maxmin: true,
			full: function(elem) {
			...
			yes:function(index,layero){
				...
				var serviceName = body.find("#serviceName").val();
				var serviceBiaoyu = body.find("#serviceBiaoyu").val();
				var serviceContent = body.find("#serviceContent").val();
				var serviceMoney = body.find("#serviceMoney").val();
				var serviceCare = body.find("#serviceCare").val();
				layer.confirm("确认添加?",function(){
					layer.alert("添加成功",function(){
						addService(serviceName,serviceBiaoyu,serviceContent,serviceCare,serviceCare);
						layer.closeAll();
					})
				})
...
//添加服务
function addService(d1,d2,d3,d4,d5){
	var html = '<div class="older" style="border:1px solid #ccc; float:left; width:350px; min-height:220px; margin:0 20px 20px 0; position: relative;">'
			 + '<img style="width:100px; height:100px; float:right; margin:5px 5px 0 0" src="../HomeCareSystemBackWeb/images/older1.png"/>'
			 + '<ul style="margin:20px 0 10px 8px;">'
			 + '<li style="font-size: 16px; font-weight: bold;">'+d1+'</li>'
			 + '<li>'+d2+'</li>'
			 + '<li>服务内容:'+d3+'</li>'
			 + '<li>收费标准：'+d4+'</li>'
			 + '<li>注：'+d5+'</li>'
			 + '<li style="position: absolute; bottom:20px; left:112px;">'
			 + '<a href="javascript:;" class="layui-btn layui-btn-small" onclick="editService(3)" style="background: #3983e1;">'
			 + '编 辑'
			 + '</a>'
...
//编辑服务
function editService(d1,d2,d3,d4,d5,d6){
	layui.use(['layer'], function() {
		var $ = layui.jquery,
			layer = parent.layer === undefined ? layui.layer : parent.layer;
		layer.open({
			type: 2,
			title: '添加服务',
			content: 'temp/addServiceProvide.html',
			btn: ['保存', '取消'],
			area: ['550px', '450px'],
			maxmin: true,
			full: function(elem) {
			...
			success: function(layero, index) {
				//成功加载iframe
				var body = layer.getChildFrame('body', index);
				body.find("#serviceName").val(d1);
				body.find("#serviceBiaoyu").val(d2);
				body.find("#serviceContent").val(d3);
				body.find("#serviceMoney").val(d4);
				body.find("#serviceCare").val(d5);
			...
			yes:function(index,layero){
				...
				layer.confirm("确认保存?",function(){
					layer.alert("保存成功",function(){
						editServiceProvide(serviceName,serviceBiaoyu,serviceContent,serviceMoney,serviceCare,d6);
						layer.closeAll();
					})
				})
...
//编辑服务
function editServiceProvide(d1,d2,d3,d4,d5,d6){
	var html = '<img style="width:100px; height:100px; float:right; margin:5px 5px 0 0" src="../HomeCareSystemBackWeb/images/older1.png"/>'
			 + '<ul style="margin:20px 0 10px 8px;">'
			 + '<li style="font-size: 16px; font-weight: bold;">'+d1+'</li>'
			 + '<li>'+d2+'</li>'
			 + '<li>服务内容：'+d3+'</li>'
			 ...
			 + '</li>'
			 + '</ul>'
		$("#service").find(".older").eq(d6).html(html);
}
