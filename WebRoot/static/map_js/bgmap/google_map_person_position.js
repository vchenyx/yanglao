...
function loadSpeedPeopleLocation(){
	...
	$.each(m_speed_people_map, function(key, values) {
		...
		var userid = key;  //用户id
		//获取定位值;
		var east_lontitude = values.lontitudeMap; //东经
		var north_latitude= values.latitudeMap;//北纬
		var netType = values.netType;           //网络类型
		var locType = values.locType;           //定位类型
		var entryTime = values.entryTime;  //时间
		var departmentname = values.departmentname;  //部门
		var peoplename = values.peoplename;  //人名
		var phonenum = values.phonenum;  //手机号码
		var peopleId = values.peopleId;  //工号
		...
		if(locType < 1){
			...
			loc_type = "SOS求助";
			baseIcon.iconSize = new GSize(55, 33);  
			var myicon = new GIcon(baseIcon);  
		...
		//创建窗口
		var content = "<table>"+
		"<tr><td>人员编号：</td><td><b>"+peopleId+"</b></td></tr>" +
    	"<tr><td>所属部门：</td><td>"+departmentname+"</td></tr>" +
    	"<tr><td>人员姓名：</td><td>"+peoplename+"</td></tr>" +
    	"<tr><td>联系电话：</td><td><b>"+phonenum+"</b></td></tr>" +
    	"<tr><td>定位方式：</td><td>"+netType+"</td></tr>" +
    	"<tr><td>定位类型：</td><td>"+loc_type+"</td></tr>" +
