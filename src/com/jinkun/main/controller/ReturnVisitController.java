...
public class ReturnVisitController {
	...
	public String getUserList(HttpSession session,HttpServletRequest request){
		...
		subwhere += " and returnVisitState = 0";
	...
	@SuppressWarnings("unchecked")
	@RequestMapping("/saveReturnVisit.do")
	@ResponseBody
	public JSONObject saveReturnVisit(HttpSession session,Integer id,Integer selectId,String returnVisitPeople,Long returnVisitDate) throws Exception {
		JSONObject json = new JSONObject();
		List<FComplaintInfo> findByHql = commonService.findByHql(FComplaintInfo.class, "id = "+id, null);
		FComplaintInfo fcomplaintInfo =(FComplaintInfo) commonService.findByIntegerId(FComplaintInfo.class, id);
		fcomplaintInfo.setReturnVisitPeople(returnVisitPeople);
		fcomplaintInfo.setReturnVisitDate(returnVisitDate);
