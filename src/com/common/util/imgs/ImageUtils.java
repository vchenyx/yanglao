 ...
 public class ImageUtils {
     ...
     * @param args
     ...
     public static void main(String[] args) {
        ...
        // 方法一：按指定起点坐标和宽高切割,居中
        ImageUtils.cut4("e:/abc.jpg", "e:/abc_cut.jpg");//测试OK
     ...
     */
      ...
      * @param height 目标切片高度
      */
     public final static void cut5(String srcImageFile, String result) {
    	 try {
    		 // 读取源图像
    		 BufferedImage bi = ImageIO.read(new File(srcImageFile));
    		 int srcWidth = bi.getWidth(); // 源图宽度
    		 ...
    		 if (srcWidth > 0 && srcHeight > 0) {
    			 ...
    			 if (srcImageFile.substring(srcImageFile.lastIndexOf(".")).contains("png")) {
    				 //如果是png使用argb的通道，否则背景变黑
    				 tag = new BufferedImage(srcWidth, srcHeight, BufferedImage.TYPE_INT_ARGB);
    			 } else {
      ...
      * @param srcImageFile 源图像地址
      * @param result 切片后的图像地址
      * @param x 目标切片起点坐标(srcWidth-width)/2
      * @param y 目标切片起点坐标0
      * @param width 目标切片宽度
      * @param height 目标切片高度
      */
     public final static void cut4(String srcImageFile, String result) {
    	 try {
    		 // 读取源图像
    		 BufferedImage bi = ImageIO.read(new File(srcImageFile));
    		 int srcWidth = bi.getWidth(); // 源图宽度
